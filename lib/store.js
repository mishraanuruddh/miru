...
const DEFAULTS = {
  ...
  spritePack: 'sticker', // classic | kawaii | sticker | any loaded pack id
  skin: 'white',
  customColors: null, // {headL,headR,muzzle,innerEar,nose,body,chest,paws,tail,tailTip,iris,pupil}
  pixelOverrides: null, // per pack: {packId: {"x,y": "#hex"}} on that pack's front frames
  scale: 4,
  openAtLogin: false,
  reactions: {
    eyeFollow: true,
  ...
  voice: {
    ...
    agentEnter: true,    // press Enter after typing (submits question answers; off = review first)
...
class Store {
  constructor(dir) {
    ...
    try {
      if (fs.existsSync(this.file)) {
        ...
        // v5: sprite sets became packs — the default look is the sticker set,
        // classic stays classic, and painted pixels move under their pack ids
        if ((saved.version || 1) < 5) {
          if (!saved.spritePack) {
            saved.spritePack = saved.spriteStyle === 'classic' ? 'classic' : 'sticker';
          }
          const ov = saved.pixelOverrides;
          if (ov && Object.keys(ov).some((k) => k.includes(','))) {
            // flat pre-pack paint visibly applied under both grids; keep both
            saved.pixelOverrides = { classic: { ...ov }, sticker: { ...ov } };
          }
          delete saved.spriteStyle;
        }
        saved.version = 5;
  ...
  // complete any pending debounced write now; temp file + rename so a crash
