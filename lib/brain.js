...
// find the user's claude binary: PATH first, then the usual homebrew spots
function resolveClaude() {
  const candidates = (process.env.PATH || '').split(path.delimiter)
    .filter(Boolean)
    .map((dir) => path.join(dir, 'claude'))
    .concat(['/opt/homebrew/bin/claude', '/usr/local/bin/claude']);
  for (const p of candidates) {
    try { if (fs.existsSync(p)) return p; } catch (e) { /* unreadable dir */ }
  }
  return null;
}
...
let claudeBin; // undefined = not probed yet; null = not found
function availability() {
  if (claudeBin === undefined) claudeBin = resolveClaude();
  return { claude: !!claudeBin };
}
