...
const MARKER = 'miru';
const LEGACY_MARKER = 'pixelpaw'; // pre-rename installs remain ours
...
function findNotify(text) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (/^notify\s*=/.test(t)) {
      return { exists: true, index: i, ours: t.includes(MARKER) || t.includes(LEGACY_MARKER), line: t };
    }
  }
  return { exists: false };
}
...
function validPort(p) { return Number.isInteger(p) && p > 0 && p < 65536; }

function install(port) {
  // the port comes from user-editable settings.json and lands in shell commands
  if (!validPort(port)) return { ...status(port), error: 'invalid agent port: ' + port };
  const text = read();
  ...
  fs.copyFileSync(CONFIG_PATH, CONFIG_PATH + '.miru-backup');
  const lines = text.split('\n');
  if (n.exists) {
    lines[n.index] = line;
  } else {
