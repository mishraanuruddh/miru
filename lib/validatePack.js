...
const RESERVED_IDS = ['classic', 'kawaii', 'sticker', 'constructor', 'prototype'];
const MAX_DIM = 48;
const MAX_FRAMES = 64;
...
// custom region names may not shadow skin keys — resolveSkin lets the base
// skin win, so a shadowed name either goes silently dead or retunes patterns
const RESERVED_REGION_NAMES = new Set([
  ...Object.values(REGION_OF), 'iris', 'pupil', 'rim', 'patternColor', 'label',
]);
...
function validatePack(id, raw) {
  ...
  if (RESERVED_IDS.includes(id)) return { pack: null, errors: [`"${id}" is a reserved pack id`] };
  ...
  if (raw.regions != null) {
    if (!isObj(raw.regions)) err('regions must be an object of {char: regionName}');
      ...
      if (RESERVED_REGION_NAMES.has(name)) { err(`region name "${name}" belongs to the skin — pick another`); continue; }
      regions[ch] = name;
    }
  }
  ...
  if (raw.anims != null) {
    if (!isObj(raw.anims)) err('anims must be an object');
      ...
      if (!isObj(g) || typeof g.frame !== 'string' || !frames[g.frame]) {
        ...
        anims.giftPresent = { frame: g.frame };
      }
    }
  }
