...
async function typeKeys(text, { pressEnter = false } = {}) {
  const safe = String(text).replace(/[^a-zA-Z0-9 ]/g, '').slice(0, 8);
  if (!safe) return { ok: false };
  // the small delay lets the picker register the digit before Enter submits
  const script = `delay 0.3\ntell application "System Events" to keystroke "${safe}"` +
    (pressEnter ? '\ndelay 0.15\ntell application "System Events" to key code 36' : '');
  const r = await osa(script);
  return { ok: r.ok, error: r.ok ? undefined : r.out };
}

// Type arbitrary text into the frontmost app — used for voice commands to an
// agent terminal, ONLY ever after an explicit user confirmation. By default
// the text is left un-entered in the prompt so the user can review it.
async function typeText(text, { pressEnter = false, maxLen = 200 } = {}) {
  const t = String(text)
    .replace(/[\r\n\t]/g, ' ')
  ...
  const esc = t.replace(/\\/g, '\\\\').replace(/"/g, '\\"'); // backslashes first
  const lines = [
    'delay 0.35',
    `tell application "System Events" to keystroke "${esc}"`,
  ];
  if (pressEnter) lines.push('tell application "System Events" to key code 36');
  const r = await osa(lines.join('\n'));
  return { ok: r.ok, error: r.ok ? undefined : r.out };
}

module.exports = { focusTty, typeKeys, typeText };
